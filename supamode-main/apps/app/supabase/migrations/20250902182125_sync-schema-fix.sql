...
-- Update policy to allow self-modification
alter policy "update_accounts" on "supamode"."accounts" to authenticated using (
    (id = supamode.get_current_user_account_id()) OR
    supamode.can_action_account(id, 'update'::supamode.system_action)
);

-- Drop no longer required triggers and functions
drop trigger prevent_update_auth_user_id on supamode.accounts;
drop function supamode.prevent_update_auth_user_id ();