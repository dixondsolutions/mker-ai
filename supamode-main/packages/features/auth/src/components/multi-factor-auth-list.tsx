  ...
  if (isLoading) {
    return (
      <div className={'flex items-center space-x-4'}>
        ...
        <div>
          <Trans i18nKey={'settings:authentication.loadingFactors'} />
        </div>
      </div>
    );
  }
    ...
    return (
      <div>
        <Alert variant={'destructive'}>
          ...
          <AlertTitle>
            <Trans i18nKey={'settings:authentication.factorsListError'} />
          </AlertTitle>

          <AlertDescription>
            <Trans
              i18nKey={'settings:authentication.factorsListErrorDescription'}
            />
          </AlertDescription>
        </Alert>
      </div>
    ...
    return (
      <div className={'flex flex-col space-y-4'}>
        <Alert>
          ...
          <AlertTitle>
            <Trans i18nKey={'settings:authentication.multiFactorAuthHeading'} />
          </AlertTitle>
      ...
      const promise = unEnroll.mutateAsync(factorId).then((response) => {
        ...
        if (!response.success) {
          ...
          throw t(`auth:errors.${errorCode}`, {
            defaultValue: t(`settings:authentication.unenrollFactorError`),
          });
        }
      });

      toast.promise(promise, {
        loading: t(`settings:authentication.unenrollingFactor`),
        success: t(`settings:authentication.unenrollFactorSuccess`),
        error: (error: string) => {
          return error;
        },
      });
    ...
    <AlertDialog open={!!props.factorId} onOpenChange={props.setIsModalOpen}>
      <AlertDialogContent>
        <AlertDialogHeader>
          <AlertDialogTitle>
            <Trans
              i18nKey={'settings:authentication.unenrollFactorModalHeading'}
            />
          </AlertDialogTitle>

          <AlertDialogDescription>
            <Trans
              i18nKey={'settings:authentication.unenrollFactorModalDescription'}
            />
          </AlertDialogDescription>
        </AlertDialogHeader>
        ...
        <AlertDialogFooter>
          ...
          >
            <Trans
              i18nKey={'settings:authentication.unenrollFactorModalButtonLabel'}
            />
          </AlertDialogAction>
        </AlertDialogFooter>
      </AlertDialogContent>
  ...
  return (
    <Table>
      <TableHeader>
        <TableRow>
          <TableHead>
            <Trans i18nKey={'settings:authentication.factorName'} />
          </TableHead>
          <TableHead>
            <Trans i18nKey={'settings:authentication.factorType'} />
          </TableHead>
          <TableHead>
            <Trans i18nKey={'settings:authentication.factorStatus'} />
          </TableHead>
            ...
            <td className={'flex justify-end'}>
              <TooltipProvider>
                <Tooltip>
                  ...
                  <TooltipContent>
                    <Trans
                      i18nKey={'settings:authentication.unenrollTooltip'}
                    />
                  </TooltipContent>
                </Tooltip>
              </TooltipProvider>
