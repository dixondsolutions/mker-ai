...
export function Sidebar(props: {
  ...
  const expandOnHover =
    props.expandOnHover ??
    import.meta.env['EXPAND_SIDEBAR_ON_HOVER'] === 'true';
...
export function SidebarContent({
  ...
  const className = cn('gap-1.5.5 flex w-full flex-col py-1', customClassName, {
    'px-4': !collapsed,
    'px-2': collapsed,
  });
  ...
  return (
    ...
    >
      ...
      <If condition={collapsible ? !isGroupCollapsed : true}>
        <div id={id} className={'gap-1.5.5 flex flex-col'}>
          {children}
        </div>
      </If>
    </div>
