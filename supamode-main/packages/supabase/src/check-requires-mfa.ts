...
/**
 * @name checkIsMfaEnabled
 * @description Checks if the current session has multi-factor authentication enabled.
 * @param client
 */
export async function checkIsMfaEnabled(client: SupabaseClient) {
  const assuranceLevel = await client.auth.mfa.getAuthenticatorAssuranceLevel();
  ...
  const { currentLevel } = assuranceLevel.data;

  return currentLevel === ASSURANCE_LEVEL_2;
}
