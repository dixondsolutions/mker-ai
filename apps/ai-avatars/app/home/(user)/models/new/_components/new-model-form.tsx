...
function UploadImageForm({ onUpload }: { onUpload: (files: File[]) => void }) {
  ...
  return (
    <div
      ...
      className={cn(
        'hover:bg-secondary-50 hover:border-primary flex h-60 w-60 flex-1 cursor-pointer flex-col items-center justify-center rounded-md border border-dashed p-8 transition-colors',
        {
      ...
      >
        {files.map((file, index) => {
          return (
            <div
              key={file.name}
              className={'animate-in fade-in relative h-48 w-48'}
            >
              <TooltipProvider>
                <Tooltip>
