...
export function KanbanBoard(props: KanbanBoardProps) {
  ...
  return (
    <KanbanEventsContext.Provider value={events}>
      <div
        className={
          'pb-container z-0 flex flex-1 overflow-x-auto overflow-y-hidden border border-transparent'
        }
      ...
      >
        {(provided: DroppableProvided, snapshot) => (
          <>
            <div
              className={cn(
                `animate-in fade-in fixed top-0 left-0 h-screen w-screen transition-all`,
                {
...
function KanbanColumn({
  ...
  return (
    <div className={cn('h-full w-full lg:max-w-[360px] lg:min-w-[360px]')}>
...
function KanbanTask({ task }: { task: KanbanTaskType }) {
  ...
  return (
    <div
      ...
      className={`group bg-background dark:hover:shadow-primary/5 flex cursor-pointer flex-col space-y-2 rounded-lg border p-4 shadow-sm transition-all hover:shadow-lg dark:border`}
    >
...
function EditableColumnName(
  ...
  return (
    <span
      ...
      ref={ref}
      className={
        'hover:bg-muted focus:border-border min-w-[20px] border border-transparent font-semibold transition-colors outline-none'
      }
      contentEditable
      onBlur={(e) => {
